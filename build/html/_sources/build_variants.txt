...
libraries in the space (`STATIC` or `SHARED`) x (`DEBUG` or `RELEASE`)
x (`SINGLE_THREADED` or `MULTI_THREADED`), resulting in 8 different
...
build also creates permutations with (`STATIC_RUNTIME` or
`DYNAMIC_RUNTIME`). Certain non-sensical combinations of libraries
